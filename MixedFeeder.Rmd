...
```{r transientMixFeed, echo=FALSE, fig.height=15, fig.width=10}
plot_transient()
```


```{r, include=FALSE}
save.image(file="MixedFeeder.RData")
```
...
```{r equiMixFeed100, echo=FALSE, fig.height=8, fig.width=5}
plot_equilibrium(ylim.H=5)
```
...
```{r transientMixFeed100, echo=FALSE, fig.height=15, fig.width=10}
plot_transient()
```

```{r, include=FALSE}
save.image(file="MixedFeeder100.RData")
```
