...
eqveg.fct <- function(g, init.params, par.name, par.clim)
{
  ...
  equi.veg = equi_veget(alpha=fparams["aT"], c = fparams["c0"], k = fparams["k"], deltaT = fparams["dT"], deltaB = fparams["dB"])
  names(equi.veg) = c("T", "R", "B")
  if(min(equi.veg >0))
  {
    require(rootSolve)
...
simulation <- function(newEq, grad.start, grad.pars, simu.time, woH = FALSE, init.params, model)
{
  ...
  colnames(mat) = c("T", "R", "B", "H")
  mat$V = 1-mat$T - mat$R - mat$B
  return(mat)
}
...
applyAnalysis <- function(init.params, grad.div, gradient,par.clim, par.name, model)
{
  ...
  # dominance
  #--------------------------------
  open = apply(newEq.all[,c("V","R")],1,sum)
  dominance = c("T","B", "V")[apply(cbind(newEq.all[,c("T","B")],open), 1, which.max)]
  dominance.veg = c("T","B", "V")[apply(cbind(newEq.veg[,c("T","B")],open), 1, which.max)]
  ...
  stats = list(newEq.veg=newEq.veg, newEq.all=newEq.all, osc=oscillations,
               ...
               deltaN.all=deltaN.all , deltaT.all=deltaT.all,
